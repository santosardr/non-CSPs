...
then
	...
	then
	    ...
	    deld1=$[$rand  + 285 ];
            rand=$(randomgenerator)
            delc2=$[$rand + 1000];
	    run=$(echo "$dela1$delb1$dela2$delb2$delc1$delc2")
            printf  "\n%d,%d,%d,%d" $dela1 $delb1 $delc1 $deld1 >> group$1.result	    
	    sed "$[$dela1]d;$[$delb1]d;$[$delc1]d;$[$deld1]d" training.arff > $run-training.arff
